    ...
    <section id='footer' className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
       <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className='flex flex-1 flex-col justify-start mr-10'>
          <img src={logo} className="w-[266px] h-[72px] object-contain" alt="" />
            ...
            <div key={footerLink.id} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className='list-none mt-4'>
                {footerLink.links.map((link, index)=>{
                  return(
                  <li key={link.name} className={`${index !== footerLink.links.length - 1 ? "mb-4": "mb-0"} font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer`}>{link.name}</li>
                  );
                })}
